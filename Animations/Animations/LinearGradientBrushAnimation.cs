...
namespace Animations
{
    public class LinearGradientBrushAnimation : LinearGradientBrushAnimationBase
    {
        ...
        public LinearGradientBrushAnimation(LinearGradientBrush fromValue, LinearGradientBrush toValue, Duration duration)
            : this()
        {
            this.From = fromValue;
            this.To = toValue;
        ...
        protected override LinearGradientBrush GetCurrentValueCore(LinearGradientBrush defaultOriginValue, LinearGradientBrush defaultDestinationValue, AnimationClock animationClock)
        {
            ...
            brush.StartPoint = From.StartPoint + ((To.StartPoint - From.StartPoint) * (double)animationClock.CurrentProgress);
            brush.EndPoint = From.EndPoint + ((To.EndPoint - From.EndPoint) * (double)animationClock.CurrentProgress);
            ...
            for (int cnt = 0; cnt < From.GradientStops.Count; cnt++)
            {
                ...
                newColor = Color.Multiply(newColor, (float)animationClock.CurrentProgress);
                newColor = Color.Add(newColor, color1);

                // calc offset
                double offset1 = (double)stop1.Offset;
                double offset2 = (double)stop2.Offset;
                double offset = offset1 + ((offset2 - offset1) * (double)animationClock.CurrentProgress);
